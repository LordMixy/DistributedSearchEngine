...
arena_chunk_t* arena_chunk_init(ptrdiff_t size)
{
	printf("arena_chunk_init: requested block of size %.2fMB\n", ((double) size / (1024.0 * 1024.0)));

    arena_chunk_t* arena_chunk = malloc(sizeof(arena_chunk_t) + size);
    if (arena_chunk) {
        arena_chunk->size = size;
        arena_chunk->used = 0;
        arena_chunk->next = NULL;
...
{   
	// static int i = 0;  
	// printf("%d requested allocation of size: %lu\n", ++i, size);
    // ------------------
    // |  used  |  ...  |
    // ------------------
    // |        |         
    ...
    if (arena->current->used + needed > arena->current->size) {
        ...
        // Si crea un altro chunk; needed = size + align?, arena->chunks->size * 1.5
        arena_chunk_t* chunk = arena_chunk_init(UTILS_MAX(arena->chunks->size * 1.5, needed));
        if (!chunk) {
            arena->flags |= ARENA_CHUNK_FAILED;
            return NULL;
        }
